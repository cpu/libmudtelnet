...
use libmudtelnet::compatibility::Entry;
use libmudtelnet::events::{Iac, Negotiation, Subnegotiation};
...
impl SimulationEvents {
    fn to_bytes(&self) -> Bytes {
        ...
        match self {
            SimulationEvents::IAC(iac) => iac.clone().into(),
            SimulationEvents::Negotiation(neg) => neg.clone().into(),
            SimulationEvents::Subnegotiation(sub) => sub.clone().into(),
...
fn bench_receive(b: &mut Bencher) {
    ...
    b.iter(|| {
        ...
        for _ in 0..INPUT_EVENTS {
            let input_event =
                SimulationEvents::arbitrary(&mut Unstructured::new(&data_buf)).unwrap();
