...
use libmudtelnet::events::{Event, Subnegotiation};
...
fn handle_events(event_list: Vec<Event>) -> CapturedEvents {
    let mut events = CapturedEvents::default();
    for event in event_list {
        match event {
            Event::Iac(ev) => {
                println!("IAC: {}", ev.command);
                events.push(EventKind::Iac);
            }
            Event::Negotiation(ev) => {
                println!("Negotiation: {} {}", ev.command, ev.option);
                events.push(EventKind::Negotiation);
            }
            Event::Subnegotiation(ev) => {
                println!("Subnegotiation: {} {:?}", ev.option, ev.buffer);
                events.push(EventKind::Subnegotiation);
            }
            Event::DataReceive(buffer) => {
                println!(
                    "Receive Data: {}",
                    std::str::from_utf8(&buffer[..]).unwrap_or("Bad utf-8 bytes")
                );
                events.push(EventKind::Recv);
            }
            Event::LineReceive(buffer) => {
                println!(
                    "Receive Line: {}",
                    std::str::from_utf8(&buffer[..]).unwrap_or("Bad utf-8 bytes")
                );
            ...
            Event::DataSend(buffer) => {
                println!("Send: {:?}", buffer);
                events.push(EventKind::Send);
            }
            Event::DecompressImmediate(buffer) => {
                println!("DECOMPRESS: {:?}", buffer);
                events.push(EventKind::Decom);
            }
        };
    }
    events
...
fn test_parser() {
    ...
    if let Some(ev) = instance._will(opt::GMCP) {
        assert_eq!(handle_events(vec![ev]), events![EventKind::Send]);
    }
    ...
    assert_eq!(
        ...
        events![EventKind::Recv, EventKind::Iac]
    );
    assert_eq!(
    ...
    assert_eq!(
        handle_events(
            instance.receive(&[&[cmd::IAC, cmd::DO, 200][..], b"Some random data"].concat())
        ),
        events![EventKind::Send, EventKind::Recv]
    );
    let gmcp_subneg: Bytes = Subnegotiation {
        option: opt::GMCP,
        buffer: Bytes::copy_from_slice(b"Core.Hello {}"),
    }
    .into();
    assert_eq!(
        handle_events(instance.receive(&gmcp_subneg)),
        events![EventKind::Subnegotiation]
    );
    assert_eq!(
        handle_events(
            instance.receive(
        ...
        events![EventKind::Subnegotiation, EventKind::Recv, EventKind::Iac]
    );
    let mccp_subneg: Bytes = Subnegotiation {
        option: opt::MCCP2,
        buffer: Bytes::copy_from_slice(b" "),
    }
    .into();
    assert_eq!(
        handle_events(
        ...
        events![EventKind::Subnegotiation, EventKind::Decom]
    );
    assert_eq!(
        // TODO(@cpu): Can data be made easier to understand at a glance?
        handle_events(instance.receive(&[
            87, 104, 97, 116, 32, 105, 115, 32, 121, 111, 117, 114, 32, 112, 97, 115, 115, 119,
            111, 114, 100, 63, 32, 255, 239, 255, 251, 1
        ])),
        events![EventKind::Recv, EventKind::Iac, EventKind::Send]
    );
}
...
fn test_subneg_separate_receives() {
    ...
    assert_eq!(handle_events(events), events![EventKind::Subnegotiation]);
...
fn test_subneg_utf8_content() {
    ...
    // buffer contents.
    if let Event::Subnegotiation(sub) = events.first().unwrap() {
...
fn test_bad_subneg_dbuffer() {
    ...
    let entry = Entry::new(true, false, true, false);
    let opts = Table::from_options(&[(cmd::IAC, entry.into())]);
...
#[test]
fn test_into_bytes() {
    let bytes: Bytes = libmudtelnet::events::Iac { command: cmd::IAC }.into();
...
mod compat_tests {
    ...
    #[test]
    fn test_parser_diff11() {
        test_app(&TelnetApplication {
            options: vec![(250, 138)],
            received_data: Vec::default(),
        })
    }

    #[test]
    fn test_parser_diff12() {
        test_app(&TelnetApplication {
            options: vec![(253, 93)],
            received_data: vec![vec![253, 255, 255, 255, 254, 253]],
        })
    }
    ...
    fn test_wtf() {
        ...
        const DEFINED_FLAGS: u8 = ENABLED_LOCAL | ENABLED_REMOTE | LOCAL_STATE | REMOTE_STATE;

        assert_eq!(93 & DEFINED_FLAGS, 13);
    }
}
...
mod tokio_tests {
    ...
    use libmudtelnet::events::Event;
    use libmudtelnet::telnet::op_command::{IAC, SB, SE};
    use libmudtelnet::telnet::op_option::GMCP;
    use libmudtelnet::Parser;

    // The same content as `test_subneg_utf8_content()`, but async through the Tokio codec
    // impl.
    #[tokio::test]
    async fn test_decoder() {
        ...
        let mut reader = FramedRead::new(Cursor::new(gmcp_msg), parser);

        let events = reader.next().await.unwrap().unwrap();
        assert_eq!(events.len(), 1, "only expected one event to be parsed");
